...
function fn_pop() {
	var vData = "";
	vData = $("#dep_code_value").val();
	
	window.open('html/dep_mem/department/html/dep_new.jsp?side='+vData, '부서등록', ' width=440, height=260');
}
...
function fn_detail(obj) {
	...
	vData = data[0];
	window.open('html/dep_mem/department/html/dep_new.jsp?param='+vData, '부서등록', ' width=440, height=260');
...
function fn_search(vPage) {
	...
	$.getJSON(urlPathContext + "/searchDepartment.do?"+parmData + "&" + param, {format: 'json'}, function(data) {
		...
		vData += "<th width='9%'>부서코드</th>";
		vData += "<th width='15%'>부서명</th>";
		vData += "<th colspan='2' width='20%'>부서장</th>";
		vData += "<th width='13%'>상위부서</th>";
		vData += "<th>생성일자</th>";
		vData += "<th>소멸일자</th>";
		vData += "<th>비고</th>";
