...
<head>
	<title>휴가원 등록</title>
	<meta charset="UTF-8">
	<link href="../css/insertBreak.css" rel="stylesheet" type="text/css" media="screen" />
	<link href="http://ajax.googleapis.com/ajax/libs/jqueryui/1.8/themes/base/jquery-ui.css" rel="stylesheet" type="text/css" />
	<script type="text/javascript" src="http://ajax.googleapis.com/ajax/libs/jquery/1.7.1/jquery.min.js"></script>
	<script type="text/javascript" src="../js/insertBreak.js"></script>
	...
	<script type="text/javascript">
		...
		if (datefield.type!="date"){ //if browser doesn't support input type="date", initialize date picker widget:
	    	jQuery(function($){ //on document.ready
	    		$('#BRE_DATE').datepicker(clareCalendar);
		        $('#BRE_START_DATE').datepicker(clareCalendar);
		        $('#BRE_OVER_DATE').datepicker(clareCalendar);
	  	  });
	}
	</script>
</head>
...
<form id="dataForm" method="post">
		...
		%>
		<input type="hidden" id="BRE_CODE" name="BRE_CODE" value="<%=key%>">
		...
		<!-- 휴가원 글씨와 결재란 박스 묶어주는 div -->
		<div id ="top">
			<!-- 휴가원 글씨 -->
			<div id="lefttop">
				<font size=6 face=굴림><h1><b>휴가원</b></h1></font>
			</div>
			<!-- 결재란 테이블 -->
			<div id = "righttop">
		...
		%> 
			⚫ 작성일자 :  <input type="date" id="BRE_DATE" name="BRE_DATE" value="<%=date%>"></br>
			⚫ 소 &nbsp&nbsp&nbsp&nbsp 속 : <input type="text" id="BRE_TEAM" name="BRE_TEAM" size="10"/> 
		</div>
		
		<div id ="main2">
			⚫ 직 &nbsp&nbsp&nbsp&nbsp 책 : <input type="text" id="BRE_LEVEL" name="BRE_LEVEL" size="10"/> &nbsp&nbsp&nbsp&nbsp
			⚫ 성 &nbsp&nbsp&nbsp&nbsp 명 : <input type="text" id="BRE_NAME" name="BRE_NAME" size="10"/>
		</div>
		
		<div id="kind">
			⚫ 휴가종류 : <!-- <select id="BRE_TYPE"></select> -->
		</div>
		
		<div id ="reason">
			⚫ 휴가사유 : </br>
			<textarea rows="3" cols="100" id="BRE_REASON" name="BRE_REASON"></textarea>
		</div>
		
		<div id ="takeover">
			⚫ 인수인계 : 직책 : <select id="BRE_TAKEOVER_LEVEL" name="BRE_TAKEOVER_LEVEL"></select> 성명 : <select id="BRE_TAKEOVER" name="BRE_TAKEOVER"></select> 
		</div>
		
		<div id ="date">
			⚫ 휴가기간 : <input type="date" id="BRE_START_DATE" name="BRE_START_DATE"> ~ <input type="date" id="BRE_OVER_DATE" name="BRE_OVER_DATE">
		</div>
		
		<div id ="bre_rollback_reason">
			⚫ 반려사유 : </br>
			 <textarea rows="3" cols="100" id="ROLLBACK_REASON" name="ROLLBACK_REASON"></textarea>
		</div>
