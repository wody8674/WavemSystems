$(document).ready(function(){
	...
	fn_getMemCd();
	fn_search();
	$("#BRE_LEVEL").change(fn_selectMem_BRE);
	...
	$("#BRE_TAKEOVER_LEVEL").change(fn_selectMemCom_TAKEOVER);
});

//휴가원 팝업창 조회
function fn_search() {
	
	var vCodeVal = $("#BRE_CODE").val();
		...
		//여기서 현재 결재 처리상태에 따라 버튼 처리 및 비활성화 처리해주기 
		 if(vAppState == 'AP020' && vAppTEAM == vUserMc){
		  ...
		  //상세보기 함수불러오기  (코드값이 있고 작성자가 로그인한 사람일 경우)
			  ...
			  fn_selectUserInfo();
				if(vAppState == 'AP010'){
						$("#bre_rollback_reason").hide();
						
						$("#BRE_DATE").attr("disabled","true");
						$("#APP_DAM_LEVEL").attr("disabled","true");
						$("#APP_DAM").attr("disabled","true");
						$("#BRE_LEVEL").attr("disabled","true");
						$("#BRE_TEAM").attr("disabled","true");
						$("#BRE_NAME").attr("disabled","true");
						$("#APP_MASTER").attr("disabled","true");
						
						$("#btnSemiSave").attr("value", "수정");
						$("#btnSave").attr("value", "등록");
						
						$("#btnSemiSave").click(fn_updateBreak);
...
function fn_disabled(){
	...
	$("#BRE_LEVEL").attr("disabled","true");
	$("#BRE_NAME").attr("disabled","true");
	$("#BRE_TYPE").attr("disabled","true");
	$("#BRE_REASON").attr("disabled","true");
	$("#BRE_TAKEOVER_LEVEL").attr("disabled","true");
	$("#BRE_TAKEOVER").attr("disabled","true");
	$("#BRE_START_DATE").attr("disabled","true");
	$("#BRE_OVER_DATE").attr("disabled","true");
	$("#bre_rollback_reason").hide();
}
...
function fn_selectSession() {
	...
	$.getJSON(urlPathContext + "/sessionInfo.do", {format: 'json'}, function(data) {
		$.each(data, function(i, n) {
			$("#BRE_LEVEL").attr("value", n.userMP_NM);
			$("#BRE_TEAM").attr("value", n.userDC_NM);
			$("#BRE_NAME").attr("value", n.userNm);
...
function fn_selectUserInfo() {
	//휴가종류 함수불러오기
	fn_getConCd();
	var paramap = $("#BRE_CODE").val();
	var paramap2 = $("#APP_CODE").val();
	var paramap3 = $("#APP_DAM").val();
	var vAppState = $("#APP_STATE").val();
	
//	alert(vAppState);
	$.getJSON(urlPathContext + "/searchInsertBreak.do?key="+paramap+"&key2="+paramap2, {format: 'json'}, function(data) {
		$.each(data, function(i, n) {
			//조회된 휴가원 정보들
			$("#BRE_NAME").attr("value", n.MEM_NAME);
			// 임시저장일 경우 작성일자 현재일자로 세팅
			if (n.APP_STATE != "AP010") {
				$("#BRE_DATE").attr("value", n.BRE_DATE);
			}
			$("#BRE_TEAM").attr("value", n.DEP_NAME);
			$("#BRE_LEVEL").attr("value", n.MEM_POST_NM);
			$("#BRE_TYPE").attr("value", n.BRE_TYPE);
			$("#BRE_REASON").attr("value", n.BRE_REASON);
			$("#BRE_TAKEOVER_LEVEL").attr("value",n.MEM_POST);
			$("#BRE_TAKEOVER").attr("value", n.BRE_TAKEOVER);
			$("#BRE_START_DATE").attr("value",n.BRE_START);
			$("#BRE_OVER_DATE").attr("value",n.BRE_OVER);
...
function fn_getMemCd(){
	...
	$.getJSON(urlPathContext + "/searchPostCom.do", {format: 'json'}, function(data) {
		...
		$("#BRE_LEVEL").append(vData);
		$("#BRE_TAKEOVER_LEVEL").append(vData);
		
		fn_selectMem_BRE();
		fn_selectMemCom_DAM();
		fn_selectMemCom_TEAM();
		fn_selectMemCom_EXECUTIVE();
		fn_selectMemCom_TAKEOVER();
	});
}

//성명란에 이름 콤보박스 뿌려주기 
function fn_selectMem_BRE(){
	var memCode = $("#BRE_LEVEL").val();
...
//인수인계란에 이름 콤보박스 뿌려주기 
function fn_selectMemCom_TAKEOVER(){
	var memCode = $("#BRE_TAKEOVER_LEVEL").val();
...
function fn_getConCd() {
	$.getJSON(urlPathContext + "/searchComCode.do?SEL_CD=searchBreCom", {format: 'json'}, function(data) {
		
		var vData = "⚫ 휴가종류 :&nbsp";
		$("#kind").html("");
		
		vData += "<select id ='BRE_TYPE' name = 'BRE_TYPE'>"
...
//	if (!fn_check()) return null;
	...
	$.ajax({
		type : "POST",
		url : urlPathContext + "/saveBreak.do?btn_flag=AP010",
		data : "data=" + jsonData,
		dataType : "text",
		success : function(data) {
			if (data > "0") {
...
//	fn_validate();
	...
	$.ajax({
		type : "POST",
		url : urlPathContext + "/saveBreak.do?btn_flag=AP020",
		data : "data=" + jsonData,
		dataType : "text",
		success : function(data) {
			if (data > "0") {
...
function fn_check() {
	var vStartDate = $("#BRE_START_DATE").val();
	var vOverDate =$("#BRE_OVER_DATE").val()
	if (!((($("#APP_TEAM_LEVEL").val()).length)>0)) {
		...
		$("#APP_EXECUTIVE").focus();
		return false;
	}else if (!((($("#BRE_TYPE").val()).length)>0)) {
		alert("휴가종류를 선택하시오.");
		$("#BRE_TYPE").focus();
		return false;
	}else if (!((($("#BRE_REASON").val()).length)>0)) {
		alert("휴가사유를 입력하시오.");
		$("#BRE_REASON").focus();
		return false;
	}else if (!((($("#BRE_TAKEOVER_LEVEL").val()).length)>0)) {
		alert("인수인계란의 직책을 선택하시오.");
		$("#BRE_TAKEOVER_LEVEL").focus();
		return false;
	}else if (!((($("#BRE_TAKEOVER").val()).length)>0)) {
		alert("인수인계란의 성함을 선택하시오.");
		$("#BRE_TAKEOVER").focus();
		return false;
	}else if (!((($("#BRE_START_DATE").val()).length)>0) || !((($("#BRE_OVER_DATE").val()).length)>0)) {
		alert("휴가 기간을 선택하시오.");
		$("#BRE_START_DATE").focus();
		return false;
	}else if (vOverDate < vStartDate) {
		alert("휴가 기간을 다시 설정하시오.");
