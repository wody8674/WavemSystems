...
function fn_detail(obj){
	...
	window.open('html/approval/overtime/html/insertOvertime.jsp?key='+vKey+'&key2='+vKey2+'&key3='+vKey3+'&key4='+vKey4+'&key5='+vKey5+'&key6='+vKey6, '야근신청확인서 등록','resizable=no scrollbars=yes width=850 height=580');
...
function fn_detailBtn(obj) {
	...
	window.open('html/approval/overtime/html/insertOvertime.jsp?key='+vKey+'&key2='+vKey2+'&key3='+vKey3+'&key4='+vKey4+'&key5='+vKey5+'&key6='+vKey6, '야근신청확인서 등록', ',width=850 height=580');
}
	//*********************************************************************************************************************************************************

function fn_pop() {
	window.open('html/approval/overtime/html/insertOvertime.jsp', '야간근무신청확인서 등록','resizable=no scrollbars=yes width=850 height=580');
}
//*********************************************************************************************************************************************************
...
function fn_search() {
	...
	var parmData = makeKr($("#searchOvertime").serialize());
//	alert(parmData);
	$.getJSON(urlPathContext + "/searchOvertime.do?"+parmData, {format: 'json'}, function(data) {
		var vData = "";
...
//		vData += "	<th>번호</th>";
		...
		$.each(data, function(i, n) {
			...
			vData += "	<td><input type='hidden' value='"+n.NIG_CODE+"'>"+ (size-i) + "</td>";
			vData += "	<td><input type='hidden' value='"+n.APP_CODE+"'>"+ n.NIG_DATE +" - "+ n.NIG_NAME +"</td>";
			vData += "	<td><input type='hidden' value='"+n.APP_DAM+"'>" + n.NIG_DATE + "</td>";
			vData += "	<td><input type='hidden' value='"+n.APP_STATE+"'>" + n.NIG_NAME + "</td>";
			vData += "	<td><input type='hidden' value='"+n.APP_TEAM+"'>" + n.APP_STATE_NM + "</td>";
			vData += "	<td><input type='hidden' value='"+n.APP_EXECUTIVE+"'>" + n.NIG_TYPE_NM + "</td>";
