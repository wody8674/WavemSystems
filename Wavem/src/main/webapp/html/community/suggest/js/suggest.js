...
function fn_pop() {
	window.open('html/community/suggest/html/suggest_popup.html', '건의게시판 등록', 'width=700, height=400');
}

function fn_search() {
	
	var parmData = $("#searchSuggest").serialize();
	
	$.getJSON(urlPathContext + "/searchSuggest.do?"+parmData, {format:'json'}, function(data){
			var vData = "";
			$("#tabContain").html("");
